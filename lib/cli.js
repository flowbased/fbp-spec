...
function storeTrace(options, traceId, trace) {
  if (!options.trace) {
    return;
  }
  const date = new Date().toISOString().substr(0, 10);
  const tracePath = resolve(process.cwd(), `./.flowtrace/${date}-${traceId.replace(/\./g, '-')}.json`);
  writeFile(tracePath, JSON.stringify(trace, null, 2), (err) => {
    if (err) {
      console.error(err);
    }
    console.log(`Wrote log to ${tracePath}`);
  });
}
...
function runOptions(options, onUpdate, callback) {
  ...
  if (options.trace) {
    runnerOptions.traceCallback = (graphId, trace) => {
      storeTrace(options, graphId, trace);
    };
  }
