...
let getSuitesSync; let jsyaml; let loadYAML; let
  validate;
...
const getFileSync = function (test, callback) {
  ...
  if (stat.isDirectory()) {
    const files = ((() => {
      const result = [];
      for (const f of Array.from(fs.readdirSync(test))) {
        if (f.indexOf('.yaml') !== -1) {
...
exports.getSuites = function (tests, callback) {
  ...
  return common.asyncSeries(tests, loadTest, (err, suitesList) => {
    if (err) { return callback(err); }
    // flatten list
    const suites = [];
    for (const ss of Array.from(suitesList)) {
