...
class Runner {
  ...
  connect(callback) {
    ...
    this.prepareClient((err) => {
      ...
      const isOnline = () => {
        ...
        if (connected) { return Promise.resolve(); } return Promise.reject(new Error('Not connected to runtime'));
      };
      const tryConnect = () => {
  ...
  runTest(testcase, callback) {
    ...
    const sequence = [];
    for (let i = 0, end = inputs.length, asc = end >= 0; asc ? i < end : i > end; asc ? i++ : i--) {
      sequence.push({
        inputs: inputs[i],
        expect: expects[i],
      });
    ...
    const sendWait = (data, cb) => sendMessageAndWait(this.client, this.currentGraphId, data.inputs, data.expect, cb);
...
const runSuite = function (runner, suite, runTest, callback) {
  ...
  runner.setupSuite(suite, (err) => {
    ...
    return common.asyncSeries(suite.cases, runTest, (err) => {
      debug('testrun complete', err);

      return runner.teardownSuite(suite, (err) => {
...
exports.getComponentSuites = function (runner, callback) {
  protocol.getCapabilities(runner.client, (err, caps) => {
    ...
    return protocol.getComponentTests(runner.client, (err, tests) => {
      if (err) { return callback(err); }
...
var loadComponentSuites = function (componentTests) {
  let suites = [];
  for (const name in componentTests) {
...
const runAll = function (runner, suites, updateCallback, doneCallback) {
  const runTest = function (testcase, callback) {
    ...
    return runTestAndCheck(runner, testcase, (err, results) => {
