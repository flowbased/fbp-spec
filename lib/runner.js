...
const { FlowtraceRecorder } = require('flowtrace');
...
class Runner {
  ...
  setupSuite(suite, callback) {
    ...
    if (this.options.traceCallback && !this.recorder) {
      this.recorder = new FlowtraceRecorder(this.client);
    }
    if (!needsSetup(suite)) {
      callback(null);
      return;
  ...
  teardownSuite(suite, callback) {
    ...
    if (this.recorder && this.currentGraphId) {
      // FIXME: This returns a Promise that we should wait for
      this.recorder.stop(this.currentGraphId);
    }
    if (!needsSetup(suite)) {
      callback(null);
      return;
  ...
  runTest(testcase, callback) {
    ...
    common.asyncSeries(sequence, sendWait, (err, actuals) => {
      if (this.options.traceCallback && this.currentGraphId && this.recorder) {
        this.options.traceCallback(this.currentGraphId, this.recorder.dump(this.currentGraphId));
      }
      if (err) {
        callback(err);
        return;
...
function loadComponentSuites(componentTests, callback) {
  ...
  const tests = Object.keys(componentTests).map((name) => componentTests[name]);
  common.asyncSeries(tests, loadYAML, (err, suites) => {
    if (err) {
      callback(err);
      return;
    }
